...
@section('title', 'Volta App - Pagini app - VOLTA')
...
<div class="mobile-list-page">
    ...
    <div class="mobile-list-card__body">
      <div class="mobile-list-head">
        <div class="mobile-list-title">
          <h1>Pagini app (listă completă)</h1>
          <p>Toate paginile din aplicație, fără limitarea de tip „top câteva rânduri”.</p>
        </div>
        <form method="get" action="{{ route('mobile.analytics.pages') }}" class="mobile-list-filters">
          <div class="mobile-list-field"><label for="pagesStart">De la</label><input id="pagesStart" type="date" name="start" value="{{ $start->format('Y-m-d') }}"></div>
          <div class="mobile-list-field"><label for="pagesEnd">Până la</label><input id="pagesEnd" type="date" name="end" value="{{ $end->format('Y-m-d') }}"></div>
  ...
  <div class="mobile-list-card">
    <div class="mobile-list-card__body mobile-list-table-wrap">
      <table class="mobile-list-table">
        <thead><tr><th>Pagină</th><th>View-uri</th><th>Timp mediu</th><th>Evenimente</th></tr></thead>
        <tbody>
      ...
      @if($schemaReady && $pages)
        <div style="margin-top:12px;">{{ $pages->links('vendor.pagination.livrari') }}</div>
      @endif
    </div>
  </div>
</div>
