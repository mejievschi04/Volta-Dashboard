...
<div class="mobile-list-page">
  ...
  <div class="mobile-list-card"><div class="mobile-list-card__body"><div class="mobile-list-head">
    <div class="mobile-list-title"><h1>Bannere (listă completă)</h1><p>Toate bannerele și click-urile înregistrate pe interval.</p></div>
    <form method="get" action="{{ route('mobile.analytics.banners') }}" class="mobile-list-filters">
      <div class="mobile-list-field"><label for="bannersStart">De la</label><input id="bannersStart" type="date" name="start" value="{{ $start->format('Y-m-d') }}"></div>
      <div class="mobile-list-field"><label for="bannersEnd">Până la</label><input id="bannersEnd" type="date" name="end" value="{{ $end->format('Y-m-d') }}"></div>
  ...
  <div class="mobile-list-card"><div class="mobile-list-card__body mobile-list-table-wrap">
    <table class="mobile-list-table">
      <thead><tr><th>Banner</th><th>Click-uri</th><th>Ultimul click</th></tr></thead>
      <tbody>
      @if($schemaReady && $banners && $banners->count())
        @foreach($banners as $banner)
          <tr>
            ...
            <td>{{ $banner->last_click_at ? \Carbon\Carbon::parse($banner->last_click_at)->format('d.m.Y H:i') : '-' }}</td>
          </tr>
        @endforeach
      @else
        <tr><td colspan="3" style="color:var(--text-secondary);">Nu există date în perioada selectată.</td></tr>
      @endif
      </tbody>
    </table>
    @if($schemaReady && $banners)
      <div style="margin-top:12px;">{{ $banners->links('vendor.pagination.livrari') }}</div>
    @endif
  </div></div>
</div>
@endsection
